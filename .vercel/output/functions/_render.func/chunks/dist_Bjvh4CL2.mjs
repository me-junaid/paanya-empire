...
function toFile(fileOut, callback) {
	...
	else if (is_default.string(this.options.input.file) && nodePath.resolve(this.options.input.file) === nodePath.resolve(fileOut)) err = /* @__PURE__ */ new Error("Cannot use same file for input and output");
	else if (jp2Regex.test(nodePath.extname(fileOut)) && !this.constructor.format.jp2.output.file) err = errJp2Save();
	if (err) {
